...
NAME="Master Player,Ralph,Darkatma,Dragunov,Tempo,xRunRiot"
D=18
TAG=rl_doubles_delay_${D}_test
ROOTDIR=$(pwd)
RAYDIR="/home/kyle/ray-resources"
...
python slippi_ai/rl/run.py \
  ...
  --config.teacher="$ROOTDIR/latest_inclsingles_4750k.pkl" \
  --config.opponent.type=self \
  --config.opponent.train=True \
  --config.actor.rollout_length=360 \
