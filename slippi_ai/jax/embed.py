...
from slippi_ai.types import (
  Action, Buttons, Controller, Game, Player, Stick,
  Randall, FoDPlatforms, Item, Items, Nana,
)
...
embed_char = OneHotEmbedding(
    'Character', size=0x21, dtype=np.uint8,
...
def _base_player_embedding(
  ...
  embedding = [
      ...
      ("is_dead", embed_bool),
      ("stocks_left", FloatEmbedding("stocks_left", scale=0.25, dtype=np.uint8)),
  ]
...
def _make_nana_embedding(
  ...
  return ordered_struct_embedding("nana", nana_embedding, types.Nana)
...
class PlayerConfig:
  ...
  def make_embedding(self) -> StructEmbedding[Player]:
    ...
    if self.with_nana:
      embedding.append(('nana', _make_nana_embedding(base_embedding)))
...
embed_stage = OneHotEmbedding(
    'Stage', size=64, dtype=np.uint8,
    one_hot_policy=OneHotPolicy.EMPTY)
...
def _items_type(value: ItemsType | enum.Enum | str) -> ItemsType:
  if isinstance(value, ItemsType):
    return value
...
@dataclasses.dataclass
class ItemsConfig:
  type: ItemsType | str = ItemsType.SKIP
  mlp_sizes: tuple[int, ...] = (128, 32)

class MLPWrapper(nnx.Module, Embedding[In, Out]):
  ...
  def __init__(
    ...
    self._mlp = jax_utils.MLP(
        rngs=rngs,
        input_size=embed.size,
        features=list(output_sizes),
        activation=nnx.relu,
        activate_final=True,
    )
    self._size = output_sizes[-1]

  @property
  def dtype(self):
    return self._embed.dtype
  ...
  def __call__(self, inputs: Out) -> Array:
    return self._mlp(self._embed(inputs))

  def map(self, f, *args: Out) -> Out:
    return self._embed.map(f, *args)

  def flatten(self, struct: Out) -> Iterator[Any]:
    yield from self._embed.flatten(struct)

  def unflatten(self, seq: Iterator[Any]) -> Out:
    return self._embed.unflatten(seq)

  def decode(self, out: Out) -> In:
    return self._embed.decode(out)
...
class EmbedConfig(tp.Generic[Action]):
  ...
  def make_items_embedding(self, rngs: nnx.Rngs | None = None):
    items_type = _items_type(self.items.type)
    if items_type is ItemsType.SKIP:
      return ordered_struct_embedding("items", [], Items)

    embed_item: Embedding[Item, Any] = self.make_item_embedding()
    if items_type is ItemsType.MLP:
      if rngs is None:
        raise ValueError('ItemsType.MLP requires rngs')
      embed_item = MLPWrapper(
          rngs=rngs,
          output_sizes=self.items.mlp_sizes,
          embed=embed_item,
      )
    elif items_type is not ItemsType.FLAT:
      raise ValueError(f'Unsupported items config type: {self.items.type}')
  ...
  def make_game_embedding(self, rngs: nnx.Rngs | None = None):
    if self.num_players not in (2, 4):
      raise ValueError(f'num_players must be 2 or 4, got {self.num_players}')
    ...
    if self.with_randall_phase:
      embedding_fields.append(('randall_phase', embed_randall_phase))
    embedding_fields.append(('is_teams', embed_bool))

    if self.with_randall_xy:
      embed_xy = FloatEmbedding("randall_xy", scale=self.player.xy_scale)
      embedding_fields.append((
          'randall',
          struct_embedding_from_nt("randall", Randall(x=embed_xy, y=embed_xy)),
      ))

    if _items_type(self.items.type) is not ItemsType.SKIP:
      embedding_fields.append(('items', self.make_items_embedding(rngs=rngs)))
