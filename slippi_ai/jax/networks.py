...
from slippi_ai.jax import opponent_pooling as opponent_pooling_lib
...
class ResBlock(nnx.Module):
  ...
  def __init__(
    ...
    self.layernorm = LayerNorm(residual_size, rngs=rngs)
...
class Embeddings(tp.NamedTuple, tp.Generic[Action]):
  ...
  def init(
    ...
    embed_game = config.make_game_embedding(rngs=rngs)
...
class SimpleEmbedModule(nnx.Module, EmbedModule[Action]):
  ...
  def __init__(
    ...
    self._embedding_modules = nnx.List(
        _collect_embedding_modules(self._embed_state_action))
...
def _collect_embedding_modules(embedding: embed_lib.Embedding) -> list[nnx.Module]:
  modules: list[nnx.Module] = []
  if isinstance(embedding, nnx.Module):
    modules.append(embedding)
  if isinstance(embedding, embed_lib.StructEmbedding):
    for _, child in embedding.embedding:
      modules.extend(_collect_embedding_modules(child))
  elif isinstance(embedding, embed_lib.CompoundEmbedding):
    modules.extend(_collect_embedding_modules(embedding._embed_mid))
  elif isinstance(embedding, embed_lib.MLPWrapper):
    modules.extend(_collect_embedding_modules(embedding._embed))
  return modules
...
class SimpleEmbedNetwork(StateActionNetwork[Action]):
  ...
  def __init__(
      ...
      input_preprocessor: tp.Optional[opponent_pooling_lib.OpponentPoolingPreprocessor],
  ...
  def _embed(self, state_action: StateAction[S, Action]) -> Array:
    if self._remat:
      embedded = nnx.remat(apply)(self._embed_module, state_action)
    else:
...
def build_embed_network(
    ...
    num_names: int,
    network_config: dict,
    opponent_pooling: tp.Optional[opponent_pooling_lib.OpponentPoolingConfig] = None,
    embed_action: tp.Optional[embed_lib.Embedding[Controller, Action]] = None,
  ...
  if opponent_pooling is not None and opponent_pooling.enabled:
    input_preprocessor = opponent_pooling_lib.OpponentPoolingPreprocessor(
        rngs=rngs,
        embed_game=embed_module._embed_game,
        config=opponent_pooling,
    )
    input_size = input_preprocessor.output_size + (
        embed_module.output_size - int(embed_module._embed_game.size))
