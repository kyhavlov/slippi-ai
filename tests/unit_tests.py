...
def random_spec() -> tf_utils.ArraySpec:
  ...
  return tf_utils.ArraySpec(dtype=np.dtype(dtype), shape=shape)

def random_signature(size: int) -> tuple[tf_utils.ArraySpec | None, ...]:
  signature = []
  for _ in range(size):
...
def random_array(spec: tf_utils.ArraySpec | None):
  if spec is None:
  ...
  if np.issubdtype(spec.dtype, np.integer):
    return np.random.randint(0, 100, size=spec.shape, dtype=spec.dtype)
  if spec.dtype == np.bool_:
    return np.random.randint(0, 2, size=spec.shape, dtype=np.int32).astype(bool)
  raise ValueError(f'Unsupported dtype: {spec.dtype}')

def sample_input(signature: tuple[tf_utils.ArraySpec | None, ...]):
